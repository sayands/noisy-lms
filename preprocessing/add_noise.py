...
from all_datasets import Dataset, load_from_disk
...
class OpenAIHumanFeedbackNoise:
    def __init__(self, cfg, split) -> None:
        ...
        self.target_dataset_dir = osp.join(
            self.dataset_root_dir,
            "noise_{}".format(str(self.noise_level).replace(".", "")),
            self.type,
        )
    ...
    def load_dataset(self):
        ...
        log.info(
            "Loaded dataset with length - {}, from - {}".format(
                self.dataset_num, self.source_dataset_dir
            )
        )
    ...
    def flip_labels(self):
        ...
        noise_indices = np.random.choice(
            self.dataset_num, num_samples_to_add_noise, replace=False
        )

        log.info(
            "Flipping labels for {} percent samples - {}".format(
                self.noise_level, num_samples_to_add_noise
            )
        )
